...
export async function getAdminStats() {
  ...
  const [{ active }] = await sql`
    SELECT COUNT(*)::int active FROM tenants
    WHERE email <> ${DEMO_EMAIL} AND subscription_active = TRUE`;
  ...
  const recent = await sql`
    SELECT email, name, invite_code, free_until, created_at
    FROM tenants
  ...
  // Invite redemptions across all codes.
  const [{ redeemed }] = await sql`
    SELECT COALESCE(SUM(used_count), 0)::int redeemed FROM invite_codes`;
