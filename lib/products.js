...
import { DEMO_PRODUCTS, isDemoTenant } from "./demoData";

// Reading a catalog never throws — the public pricing page calls it, and the
// built-in demo tenant (lib/demoData.js) has no database rows behind it.
export async function listProducts(tenantId) {
  if (!hasDb() || isDemoTenant({ id: tenantId })) return DEMO_PRODUCTS;
  try {
    const sql = requireDb();
...
export async function getProduct(tenantId, productId) {
  if (!hasDb() || isDemoTenant({ id: tenantId })) {
    return DEMO_PRODUCTS.find((p) => p.id === Number(productId)) || null;
  }
  const sql = requireDb();
  const rows = await sql`
